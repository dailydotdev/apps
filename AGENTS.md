...
- **Use `feedHeader` prop for banners above feed results** - In `MainFeedLayout`, page `children` render AFTER the `<Feed>` component. To place banners/promos above feed results, pass them via the `feedHeader` prop (through `layoutProps` on the page component). Do NOT render them as page children — they will appear below all posts.
