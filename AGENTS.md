...
- **Floating actions over positioned siblings need an explicit z-index** - When stacking a control (e.g. a lightbox close button) over a sibling that uses `position: relative`/`absolute` without z-index (e.g. a `<img>` with `relative` for centering), paint order falls back to DOM order and a later sibling will cover the earlier control. Add an explicit `z-*` class (e.g. `z-1`) to the floating control. Don't rely on DOM order.
