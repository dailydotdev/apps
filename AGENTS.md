...
- **Avoid first-day reminder prompts** - For reminder/retention promos, suppress prompting on the same calendar day the user registered unless the requirement explicitly says otherwise.
- **Keep promo copy time-agnostic unless required** - Avoid hardcoding exact send times (for example, "9:00") in marketing/promo copy unless product explicitly asks to expose that value.
