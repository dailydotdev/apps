...
- **Handle pre-selected values in paginated dropdowns** - When a dropdown uses infinite scroll and a value was previously selected (e.g. from a saved integration), that value may not be in the first page of results. Auto-fetch subsequent pages until the selected item is found or no more pages remain. Never use `findIndex(...) || 0` — it silently falls back to the first item when the value isn't found; use `?? -1` so the dropdown shows the placeholder instead.
