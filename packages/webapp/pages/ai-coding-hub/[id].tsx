...
const SourceTweetCard = ({
  ...
  return (
    <article className="rounded-12 bg-surface-float px-3 py-3">
      ...
      <p
        className="font-bold leading-snug text-text-primary"
        style={{ fontSize: '15px' }}
      >
        {item.headline}
      </p>
      ...
      <p
        className="mt-2 leading-normal text-text-secondary"
      ...
      <div className="mt-3 flex items-center justify-between">
        <div
          className="flex flex-wrap items-center gap-1 text-text-quaternary"
          style={{ fontSize: '15px' }}
        >
          {item.tags.slice(0, 2).map((tag) => (
            <span
              key={`${tweetId}-${tag}`}
              className="rounded-4 bg-background-default px-1.5 py-0.5 text-[10px] font-bold uppercase leading-none text-text-tertiary"
            >
              #{normalizeTag(tag)}
            </span>
          ))}
  ...
  const cardContent = (
    <div className="flex flex-col gap-1 px-4 py-3 text-left">
      ...
      >
        ...
        <span>{categoryLabels[item.category as Category]}</span>
        <span>&middot;</span>
...
const SignalDetailPage = (): ReactElement => {
  ...
  const item = useMemo(
    () =>
      feedItems.find((feedItem) => feedItem.id === id) as FeedItem | undefined,
    [id],
  );
  ...
  return (
    <div className="relative mx-auto min-h-page w-full max-w-4xl bg-background-default">
      <NextSeo
        title={`${item.headline} // AI Coding Hub`}
      ...
      <main className="mx-auto max-w-4xl">
        ...
        {/* Source tweets carousel */}
        <div className="border-b border-border-subtlest-tertiary py-2 pl-4">
          <div
            ref={tweetsScrollerRef}
            className="no-scrollbar flex snap-x snap-mandatory gap-6 overflow-x-auto pl-10 pr-0"
          ...
          >
            {tweetIds.map((tweetId, index) => (
              ...
              >
                <SourceTweetCard item={item} tweetId={tweetId} index={index} />
              </div>
            ))}
          </div>
          ...
          <div className="mt-2 flex items-center justify-between pb-2 pl-0 pr-4 text-text-quaternary">
            <button
              type="button"
              className="flex items-center gap-1 transition-colors hover:text-text-secondary"
            >
        ...
        <div>
          ...
          <div className="border-t border-border-subtlest-tertiary px-4 py-4">
            ...
            >
              Curated from Twitter, GitHub, and the dev community. Updated
              daily.
            </Typography>
          </div>
        </div>
