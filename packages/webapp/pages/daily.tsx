...
import { featureDailyPage } from '@dailydotdev/shared/src/lib/featureManagement';
...
const DailyPage = (): ReactElement | null => {
  ...
  const { value: isDailyEnabled, isLoading } = useConditionalFeature({
    feature: featureDailyPage,
    shouldEvaluate: isAuthReady && isLoggedIn,
  });

  const isResolved = isAuthReady && !isLoading;
  const isAllowed = isLoggedIn && isDailyEnabled;
