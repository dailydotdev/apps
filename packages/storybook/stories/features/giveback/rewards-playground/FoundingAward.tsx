...
import {
  FOUNDING_AWARD,
  PATCHY_FOUNDING_AWARD,
} from './rewardsPlayground.data';
import type { FoundingAwardState } from './rewardsPlayground.data';
import { RevealDialogShell, Reveal } from './RewardReveal';

// PLAYGROUND ONLY. The journey's special first step (see rewardsPlayground.data).

const coresLabel = `${PATCHY_FOUNDING_AWARD.value.toLocaleString()} Cores`;
// The "Impact Ambassador" founding-award dog (transparent render).
const foundingDogUrl = new URL('./assets/patchy-founding.png', import.meta.url)
  .href;
...
// Scarcity as a compact pie/ring — saves vertical height vs. a bar. The ring
// fills by % claimed (same colorful gradient as the border), % in the centre,
// spots-left underneath.
...
// The claim celebration: the award pops in on its glow, +1,000 Cores, the
// founding number, and a signed note from the CEO.
...
interface FoundingAwardCardProps {
...
export const FoundingAwardCard = ({
  initialState = 'intro',
  claimedCount = 743,
  memberNumber = 744,
}: FoundingAwardCardProps): ReactElement => {
  ...
  return (
    <>
      {/* Colorful gradient border (a 1px gradient frame), matched by the
          gradient progress bar. Sold-out drops to a quiet hairline. */}
      <div
        className={classNames(
      ...
      >
        <FlexRow
          ...
          // The colorful border gradient, but at a true 8% via color-mix (the
          // Tailwind `/opacity` modifier doesn't take on these token gradient
          // stops, which is why it read full-strength before). Sits over the
          // opaque base as a faint wash.
          style={
            isSoldOut
              ? undefined
              : {
        ...
        >
          ...
          {/* Dog anchors the right side, larger now. */}
