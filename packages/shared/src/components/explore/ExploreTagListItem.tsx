...
interface ExploreTagListItemProps {
  tag: string;
  isFollowed: boolean;
  onToggleFollow: (tag: string) => void;
}

// A directory row: the topic link plus a follow control on the right. Followed
// topics keep a persistent check (so you can tell what you already follow);
// unfollowed topics reveal a subtle "+" on hover/focus.
export function ExploreTagListItem({
  tag,
  isFollowed,
  onToggleFollow,
}: ExploreTagListItemProps): ReactElement {
  return (
