...
export interface SplitShareButtonProps {
  ...
  menu: ReactNode;
  /** Rendered above the menu body, for a preview or a heading. */
  header?: ReactNode;
  /**
   * 'grid' keeps the 4-column tile grid this control was designed around;
   * 'plain' hands layout to the caller, for a menu of whole sections.
   */
  menuLayout?: 'grid' | 'plain';
  /** Overrides the dropdown width, e.g. for a menu carrying whole sections. */
  menuClassName?: string;
  /** Overrides the scrolling wrapper, e.g. to cap a tall menu's height. */
  menuScrollableClassName?: string;
  ...
  size?: ButtonSize;
  className?: string;
  /** Lets a caller react to the dropdown opening, e.g. to prepare its menu. */
  ...
  return (
    <div className={classNames('flex items-stretch', className)}>
      ...
      >
        ...
        {/* `!p-0` because the dropdown's CSS module applies `p-1.5` at the same
            specificity — a plain `p-0` loses on stylesheet order and silently
            does nothing. The padding moves onto the grid so a `header` can run
            edge to edge. */}
        <DropdownMenuContent
          align="end"
          className={classNames('!min-w-0 !p-0', menuClassName ?? 'w-72')}
          scrollableClassName={menuScrollableClassName}
        >
          ...
          {header}
          {menuLayout === 'grid' ? (
            <div className="grid grid-cols-4 justify-items-center gap-2 p-3">
              {menu}
            </div>
          ) : (
            menu
