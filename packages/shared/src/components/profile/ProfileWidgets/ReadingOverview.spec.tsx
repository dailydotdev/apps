...
import type { UserReadHistory, UserStreak, MostReadTag } from '../../../graphql/users';
...
describe('ReadingOverview component', () => {
  ...
  it('should render main content when not loading', () => {
    ...
    expect(screen.getByText('Reading Overview')).toBeInTheDocument();
    expect(screen.getByText('Learn more')).toBeInTheDocument();
    expect(screen.getByText('Posts read in the last months (16)')).toBeInTheDocument();
  });
  ...
  it('should not render tags section when mostReadTags is empty', () => {
    ...
    expect(screen.queryByText('Top tags by reading days')).not.toBeInTheDocument();
  ...
  it('should display total reads as 0 when readHistory is empty', () => {
    ...
    expect(screen.getByText('Posts read in the last months (0)')).toBeInTheDocument();
  });
  ...
  it('should render calendar heatmap with correct props', () => {
    ...
    // Check that CalendarHeatmap component is rendered
    const heatmap = container.querySelector('[data-testid="calendar-heatmap"]');
  ...
  it('should render learn more link with correct href', () => {
    ...
    expect(learnMoreLink).toHaveAttribute('href', 'https://daily.dev/migrate-user-to-streaks');
  ...
  it('should handle large numbers in readHistory', () => {
    ...
    expect(screen.getByText('Posts read in the last months (3000)')).toBeInTheDocument();
  });
  ...
  it('should handle edge case with single read entry', () => {
    ...
    expect(screen.getByText('Posts read in the last months (1)')).toBeInTheDocument();
  });
