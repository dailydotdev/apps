...
interface PinnedSignupStripProps {
  targetId: TargetId;
  className?: string;
}

// The cover card with the auth stack for anonymous visitors, tablet and up.
// The slot goes at the end of the page's content column: it keeps the last of
// the content reachable, and the card pins over the bottom of the window at
// exactly the slot's width.
export function PinnedSignupStrip({
  targetId,
  className,
}: PinnedSignupStripProps): ReactElement | null {
  const { isAuthReady, user, showLogin } = useAuthContext();
  const { logEvent } = useLogContext();
  const isTablet = useViewSize(ViewSize.Tablet);
  const shouldRender = isAuthReady && !user && isTablet;
