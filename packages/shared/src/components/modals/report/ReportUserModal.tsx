...
import { ReportEntity, SEND_REPORT_MUTATION, type ReportReason } from '../../../report';
import { Checkbox } from '../../fields/Checkbox';
import type { UserShortProfile } from '../../../lib/user';
import { useMutation } from '@tanstack/react-query';
import { gqlClient } from '../../../graphql/common';
import { CONTENT_PREFERENCE_BLOCK_MUTATION, ContentPreferenceType } from '../../../graphql/contentPreference';
...
export const ReportUserModal = ({ offendingUser, defaultBlockUser, onClose }: ReportUserModalProps) => {
    const { displayToast } = useToastNotification();
    const { user } = useAuthContext();
    const [blockUser, setBlockUser] = useState(defaultBlockUser);
    const { isPending: isBlockPending, mutateAsync: blockUserMutation } = useMutation({
        mutationFn: () => gqlClient.request(CONTENT_PREFERENCE_BLOCK_MUTATION, {
            id: offendingUser.id,
    ...
    const { isPending: isReportPending, mutateAsync: reportUserMutation } = useMutation({
        mutationFn: ({ reason, text }: { reason: ReportReason, text: string }) => gqlClient.request(SEND_REPORT_MUTATION, {
            id: offendingUser.id,
    ...
    return (
        <ReasonSelectionModal
            ...
            footer={
                <Checkbox name="blockUser" className="font-normal"
