...
describe('FunnelStepper component', () => {
  ...
  it('should complete the active step when it reports it should be skipped', () => {
    ...
    renderComponent({
      ...mockFunnel,
      chapters: [{ id: 'chapter1', steps: [skippableStep, mockStep2] }],
    });
  ...
  it('should complete a step that reported it should be skipped before the funnel reached it', () => {
    ...
    const lastStep = { ...mockStep2, id: 'step3' } as FunnelStep;
    const funnel: FunnelJSON = {
      ...mockFunnel,
      chapters: [
        { id: 'chapter1', steps: [firstStep, skippableStep, lastStep] },
      ],
    };
    const navigation = (step: FunnelStep, index: number) => ({
      back: mockBack,
      skip: mockSkip,
      navigate: mockNavigate,
    ...
    (useFunnelNavigation as jest.Mock).mockReturnValue(
      navigation(firstStep, 0),
    );
    const { rerender } = renderComponent(funnel);
    expect(mockNavigate).not.toHaveBeenCalled();

    (useFunnelNavigation as jest.Mock).mockReturnValue(
      navigation(skippableStep, 1),
    );
    rerender(
      <TestBootProvider client={client}>
        <FunnelStepper
