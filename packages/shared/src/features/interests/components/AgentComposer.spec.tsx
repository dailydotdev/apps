...
describe('AgentComposer', () => {
  ...
  describe('while a run is in flight', () => {
    it('offers a brake instead of a send', () => {
      mountComposer();

      type('raise the bar');
      fireEvent.keyDown(field(), { key: 'Enter' });

      expect(screen.getByLabelText('Stop the agent')).toBeInTheDocument();
      expect(screen.queryByLabelText('Send to agent')).not.toBeInTheDocument();
    });
