...
// LogoIcon draws the mark as two compound paths, but the thinking animations
// move each chevron stroke on its own, so the second path is split at its
// subpath boundary. Geometry is copied verbatim from svg/LogoIcon.tsx.
const leftChevron =
...
/**
 * The exploration set lives in agentThinking.css; every variant keeps the
 * intact logo on screen for part of each loop, per the brand rule.
 */
export type AgentThinkingMarkVariant =
  | 'wave'
  | 'sheen'
  | 'pulse'
  | 'blink'
