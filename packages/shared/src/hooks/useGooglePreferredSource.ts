...
export type UseGooglePreferredSource = {
  isReady: boolean;
  hasFailed: boolean;
  addPreferredSource: () => void;
};
 ...
 * change long after that, so the auto-scan would find nothing. Manual mode also
 ...
 *
 * The script is fetched on demand rather than from `<head>`, so a page that
 * never shows the ask never pays for it.
 */
export const useGooglePreferredSource = ({
  ...
  useEffect(() => {
    ...
    const onApi = (api: PreferredSourceApi) => {
      ...
      // this a script that merely loaded slowly would still be reported as
    ...
    // asking at all: callers switch to the deeplink, which needs no script.
    const fail = () => {
