/* eslint-disable no-bitwise, @typescript-eslint/no-use-before-define, @typescript-eslint/no-shadow, @typescript-eslint/naming-convention, no-underscore-dangle, no-param-reassign, @typescript-eslint/explicit-module-boundary-types, no-console */
...
export function getQueryStringOverride(
  ...
  const match = search
    ...
    .map(([, v]) => parseInt(v, 10)); // Parse the value into an integer
